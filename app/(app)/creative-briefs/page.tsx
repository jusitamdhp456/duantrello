  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-10">
        <h1 className="text-4xl font-light text-gray-700 tracking-wide">{t("briefs_title")}</h1>
        <Link href="/creative-briefs/new" className="bg-neu-base text-gray-600 px-6 py-3 rounded-full shadow-neu-convex hover:shadow-neu-concave text-sm font-bold transition-all duration-200 uppercase tracking-widest flex items-center">
          <Plus className="w-5 h-5 mr-2" /> {t("new_brief")}
        </Link>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
            ...
            <div key={b.id} className="bg-neu-base p-8 rounded-[2rem] shadow-neu-convex flex flex-col md:flex-row justify-between md:items-center">
              <div className="mb-6 md:mb-0">
                ...
                <p className="text-sm text-gray-500 mt-4 font-medium flex flex-wrap gap-3">
                  <span className="px-3 py-1 bg-neu-base shadow-neu-concave rounded-full">{t("campaign")}: {b.campaigns?.name || 'N/A'}</span>
                  <span className="px-3 py-1 bg-neu-base shadow-neu-concave rounded-full">{t("platform")}: {b.platform}</span>
                  <span className="px-3 py-1 bg-neu-base shadow-neu-concave rounded-full uppercase tracking-widest text-xs font-bold flex items-center">{t("status")}: {b.status}</span>
                </p>
                <div className="mt-6 bg-neu-base shadow-neu-concave p-4 rounded-xl max-w-2xl">
                  <p className="text-sm text-gray-600 line-clamp-2 leading-relaxed">
                    <span className="font-bold text-gray-700 uppercase tracking-wider text-xs mr-2">{t("hook")}:</span> {b.hook}
                  </p>
                </div>
              </div>
          ...
          {briefs.length === 0 && <p className="text-gray-500 text-center py-8">{t("no_data")}</p>}
        </div>
      )}
    </div>
  );

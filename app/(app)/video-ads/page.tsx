  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-10">
        <h1 className="text-4xl font-light text-gray-700 tracking-wide">{t("video_ads_title")}</h1>
        <button onClick={() => setIsAdding(true)} className="bg-neu-base text-gray-600 px-6 py-3 rounded-full shadow-neu-convex hover:shadow-neu-concave text-sm font-bold transition-all duration-200 uppercase tracking-widest flex items-center">
          <Plus className="w-5 h-5 mr-2" /> {t("new_video_ad")}
        </button>
      </div>
      ...
      {isAdding && (
        <form onSubmit={handleCreate} className="mb-10 bg-neu-base p-8 rounded-[2rem] shadow-neu-concave border-none grid grid-cols-2 gap-6 max-w-3xl">
          <div className="col-span-2">
            <label className="block text-xs font-bold uppercase tracking-widest text-gray-500 mb-2 ml-2">{t("ad_title")}</label>
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-gray-500 mb-2 ml-2">{t("brief")}</label>
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-gray-500 mb-2 ml-2">{t("platform")}</label>
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-gray-500 mb-2 ml-2">{t("version")}</label>
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-gray-500 mb-2 ml-2">{t("status")}</label>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div key={ad.id} className="bg-neu-base rounded-[2rem] shadow-neu-convex overflow-hidden flex flex-col hover:shadow-neu-concave transition-all duration-300 p-6">
              ...
              <div className="pt-6 flex-1 flex flex-col">
                ...
                <div className="mt-4 text-xs text-gray-500 space-y-3 font-medium">
                  <p className="flex items-center"><span className="px-3 py-1 bg-neu-base shadow-neu-concave rounded-md mr-2">{t("brief")}</span> <span className="text-gray-700">{ad.creative_briefs?.title || 'N/A'}</span></p>
                  <p className="flex items-center"><span className="px-3 py-1 bg-neu-base shadow-neu-concave rounded-md mr-2">{t("platform")}</span> {ad.platform} <span className="px-3 py-1 bg-neu-base shadow-neu-concave rounded-md ml-3 mr-2">{t("version")}</span> {ad.version}</p>
                </div>
          ...
          {ads.length === 0 && <p className="text-gray-500 col-span-full">{t("no_data")}</p>}
        </div>
      )}
    </div>
  );
